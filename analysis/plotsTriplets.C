...
void plotsTriplets::Loop()
{
  ...
  TH1F *hz_trueBestMatch_ele1pt  = new TH1F("hz_trueBestMatch_ele1pt", "hz_trueBestMatch_ele1pt", 50, 0., 5.);
  ...
  // chiara: ancora da controllare e fare i plot
  ...
  TH1F *hz_comb_ele1pt  = new TH1F("hz_comb_ele1pt", "hz_comb_ele1pt", 50, 0., 5.);
  ...
  float tot_ok_all_bin[8][11];
  float tot_notok_all_bin[8][11];
  float tot_ok_sel_bin[8][11];
  float tot_notok_sel_bin[8][11];
  for (int ik=0; ik<8; ik++) {             
    for (int iele2=0; iele2<11; iele2++) {      
      tot_ok_sel_bin[ik][iele2]    = 0.;
      tot_notok_sel_bin[ik][iele2] = 0.;
      tot_ok_all_bin[ik][iele2]    = 0.;
      tot_notok_all_bin[ik][iele2] = 0.;
    }}
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    // Scan
    if (bestXYsigMatch_ok_kpt>-990) {
      for (int ik=0; ik<8; ik++) {             // 0.8 - 1.5
	for (int iele2=0; iele2<11; iele2++) {      // 0.5 - 1.5
	  tot_ok_all_bin[ik][iele2]++;
	  float kpt    = 0.8+(float)ik*0.1;         
	  float ele2pt = 0.5+(float)iele2*0.1;         
	  if (bestXYsigMatch_ok_kpt>=kpt && bestXYsigMatch_ok_ele2pt>=ele2pt) tot_ok_sel_bin[ik][iele2]++;
	}}
    }
      
    if (bestXYsigMatch_notok_kpt>-990) {
      for (int ik=0; ik<8; ik++) {             // 0.8 - 1.5
	for (int iele2=0; iele2<11; iele2++) {      // 0.5 - 1.5
	  tot_notok_all_bin[ik][iele2]++;
	  float kpt    = 0.8+(float)ik*0.1;         
	  float ele2pt = 0.5+(float)iele2*0.1;         
	  if (bestXYsigMatch_notok_kpt>=kpt && bestXYsigMatch_notok_ele2pt>=ele2pt) tot_notok_sel_bin[ik][iele2]++;
	}}
    }
  ...
  for (int ik=0; ik<8; ik++) {             // 0.8 - 1.5
    for (int iele2=0; iele2<11; iele2++) {      // 0.5 - 1.5
      float kpt    = 0.8+(float)ik*0.1;         
      float ele2pt = 0.5+(float)iele2*0.1;         
      cout << "k PT > " << kpt << ", ele2 PT > " << ele2pt 
	   <<  ", eff_ok = " << tot_ok_sel_bin[ik][iele2]/tot_ok_all_bin[ik][iele2]
	   <<  ", eff_notok = " << tot_notok_sel_bin[ik][iele2]/tot_notok_all_bin[ik][iele2] << endl;
    }}
  cout << endl;
  cout << endl;
